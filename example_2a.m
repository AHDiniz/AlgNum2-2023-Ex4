function example_2a()
    ...
    bound_condition_b.condition_type = "value";
    bound_condition_b.constants = [200.0];

    output_dir = "out/example_2a";

    for i = 1 : numel(ns)
        [x, u] = bvp1d(a, b, ns(i), p_func, q_func, r_func, bound_condition_a, bound_condition_b);
        hf = figure();
        plot(x, u);
        title(sprintf("Conservação de Calor - Valor Prescrito (n = %d)", ns(i)));
